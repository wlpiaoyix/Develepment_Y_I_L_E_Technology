...
//    addNotificationObserver(MusicPlayCellRecentPlayViewStartingNotification, self, @selector(musicPlayStarting:), nil);
//    addNotificationObserver(MusicPlayCellRecentPlayViewStopNotification, self, @selector(musicStop:), nil);
}
...
- (void)playCurrentMusic{
    [PTUtilities sendGaTrackingWithScreenName:nil category:@"I_pro" action:@"Click_I_pro" label:@"recent_play" value:nil];
    if (_music) {
        if ([YLAudioPlayer sharedAudioPlayer].isPlaying) {
            if ([[YLAudioPlayer sharedAudioPlayer].currentPlayMusic isEqual:_music]) {
