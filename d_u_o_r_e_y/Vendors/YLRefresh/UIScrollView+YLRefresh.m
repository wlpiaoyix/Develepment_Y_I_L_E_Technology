...
#pragma mark - 运行时相关
static char YLRefreshHeaderViewKey;
static char YLRefreshFooterViewKey;

- (void)setHeader:(YLRefreshHeaderView *)header {
    [self willChangeValueForKey:@"YLRefreshHeaderViewKey"];
    objc_setAssociatedObject(self, &YLRefreshHeaderViewKey,
                             header,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"YLRefreshHeaderViewKey"];
}

- (YLRefreshHeaderView *)header {
    return objc_getAssociatedObject(self, &YLRefreshHeaderViewKey);
}

- (void)setFooter:(YLRefreshFooterView *)footer {
    [self willChangeValueForKey:@"YLRefreshFooterViewKey"];
    objc_setAssociatedObject(self, &YLRefreshFooterViewKey,
                             footer,
                             OBJC_ASSOCIATION_ASSIGN);
    [self didChangeValueForKey:@"YLRefreshFooterViewKey"];
}

- (YLRefreshFooterView *)footer {
    return objc_getAssociatedObject(self, &YLRefreshFooterViewKey);
}
...
//    // 1.创建新的header
//    if (!self.header) {
//        YLRefreshHeaderView *header = [YLRefreshHeaderView header];
...
//    // 1.创建新的footer
////    if (!self.footer) {
////        YLRefreshFooterView *footer = [YLRefreshFooterView footer];
...
- (void)addFooterWithTarget:(id)target action:(SEL)action
{
    // 1.创建新的footer
    if (!self.footer) {
        YLRefreshFooterView *footer = [[YLRefreshFooterView alloc] initWithFrame:CGRectMake(0, CGRectGetHeight(self.frame) - self.contentInset.top, CGRectGetWidth([[UIScreen mainScreen] bounds]), kXHLoadMoreViewHeight)];
        self.footer = footer;
        [self addSubview:footer];
        footer.displayAutoLoadMoreRefreshedMessage = PTNSLocalizedString(@"clickLoadMoreMsgKey",nil);// YLRefreshLocalizedStrings(@"clickLoadMoreMsgKey");
        footer.footerViewIsLoadingText = PTNSLocalizedString(@"LikeLoadingMsgKey",nil);//YLRefreshLocalizedStrings(@"Loading");
//        footer.footerViewLoadingEnededText = @"Loading";
        footer.preloadValue = 1000;
        footer.autoLoadMoreRefreshedCount = 2;
        self.footer.alpha = 0;
    }
...
- (void)endMoreOverWithMessage:(NSString *)message {
    [self.footer endLoadMoreRefresing];
    self.footer.noMoreDataForLoaded = YES;
    [self.footer configuraNothingMoreWithMessage:message];
}
...
{
    return self.footer.activityIndicatorView.isAnimating;
}
